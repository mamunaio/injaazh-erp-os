...
// 8
const parts = code.split('          {/* ── KPI Cards ─────────────────────────────────────────────────── */}');
if (parts.length === 2) {
  const top = parts[0];
  let bottom = '          {/* ── KPI Cards ─────────────────────────────────────────────────── */}\n' + parts[1];
  
  // Wrap the bottom part
  bottom = `        {activeTab === 'expenses' ? (\n          <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-8">\n` + 
           bottom.replace(
             `        {/* ── Upload Receipt Modal ─────────────────────────────────────── */}`,
             `          </motion.div>\n        ) : (\n          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>\n            <BorrowLendTab initialLoans={initialLoans} />\n          </motion.div>\n        )}\n\n        {/* ── Upload Receipt Modal ─────────────────────────────────────── */}`
           );
           
  fs.writeFileSync('app/daily-expenses/DailyExpensesClient.tsx', top + bottom, 'utf-8');
