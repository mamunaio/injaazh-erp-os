  ...
  return (
    <div className="relative w-full" ref={selectRef}>
      <div 
        ...
        className={`w-full flex items-center justify-between cursor-pointer select-none bg-slate-50 dark:bg-[#09090B] border border-slate-200 dark:border-[#232734] rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white focus-within:border-indigo-500/50 transition-all ${className}`}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute z-[100] w-full bg-white dark:bg-[#11131A] rounded-xl py-2 shadow-2xl border border-slate-200 dark:border-[#232734] overflow-hidden ${dropdownUp ? 'bottom-full mb-2' : 'top-full mt-2'}`}
          >
              ...
              <div 
                ...
                className={`px-4 py-2.5 text-sm cursor-pointer transition-all flex items-center gap-2 ${
                  value === opt 
                    ...
                    : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-[#232734]/50'
                }`}
              >
...
const InputField = ({ label, icon: Icon, type = "text", value, onChange, placeholder = "", disabled = false }: any) => (
  <div className="relative group w-full">
    ...
    <input 
      ...
      disabled={disabled}
      className="w-full bg-slate-50 dark:bg-[#09090B] border border-slate-200 dark:border-[#232734] rounded-xl px-4 py-3 text-sm font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all placeholder-slate-600 disabled:opacity-50"
    />
  </div>
);
...
export default function LeadDetailsModal({ 
  ...
  const getLogIcon = (method: string) => {
    switch (method) {
      ...
      default: return <FileText size={14} className="text-slate-500 dark:text-slate-400" />;
    }
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-6 lg:p-10">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-7xl h-full md:h-[90vh] bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] md:rounded-[32px] flex flex-col overflow-hidden shadow-2xl"
        >
          {/* Header */}
          <div className="flex justify-between items-start p-6 md:p-8 border-b border-slate-200 dark:border-[#232734] bg-slate-50 dark:bg-[#09090B]">
            <div className="flex items-center gap-5">
              <div className="w-16 h-16 rounded-[20px] bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] flex items-center justify-center text-slate-700 dark:text-slate-300 font-bold text-xl shadow-inner shrink-0 relative overflow-hidden">
              ...
              <div className="flex flex-col gap-1.5">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
                  ...
                  <div className={`px-2.5 py-1 text-[10px] font-bold rounded-lg border flex items-center gap-1.5 shadow-sm ${
                    ...
                    'text-slate-500 dark:text-slate-400 border-slate-200 dark:border-[#232734] bg-white dark:bg-[#11131A]'
                  }`}>
            ...
            <button onClick={onClose} className="p-2.5 bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm">
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
            ...
            <div className="w-full lg:w-1/2 overflow-y-auto custom-scrollbar border-r border-slate-200 dark:border-[#232734] bg-slate-50 dark:bg-[#09090B]">
              <form id="lead-details-form" onSubmit={handleSave} className="p-6 md:p-8 space-y-8">
                ...
                <section>
                    ...
                    <div className="bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] p-5 rounded-[24px] space-y-5">
            ...
            <div className="w-full lg:w-1/2 flex flex-col bg-white dark:bg-[#11131A]">
                
                {/* Notes Input Area */}
                <div className="p-6 md:p-8 border-b border-slate-200 dark:border-[#232734] bg-white dark:bg-[#11131A] shrink-0">
                    <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <FileText size={14} /> Add Note or Activity
                    </h3>
                    <div className="bg-slate-50 dark:bg-[#09090B] border border-slate-200 dark:border-[#232734] rounded-2xl p-4 focus-within:border-indigo-500/50 transition-all shadow-sm">
                        <textarea
                            value={newLog.note}
                            onChange={(e) => setNewLog({ ...newLog, note: e.target.value })}
                            placeholder="Log a call, meeting, or note..."
                            className="w-full bg-transparent text-sm font-medium text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none resize-none min-h-[80px]"
                        />
                        <div className="flex items-center justify-between pt-3 mt-2 border-t border-slate-200 dark:border-[#232734]">
                            <div className="flex items-center gap-2">
                                {['Note', 'Email', 'Call', 'Meeting'].map(type => (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                                            newLog.type === type 
                                            ? 'bg-slate-200 dark:bg-[#232734] border-slate-500 text-slate-900 dark:text-white' 
                            ...
                            <button
                                ...
                                className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white text-xs font-bold rounded-xl transition-all shadow-sm disabled:opacity-50"
                            >
                                Post Note
                            </button>
                        </div>
                ...
                {/* Activity Timeline */}
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 bg-slate-50 dark:bg-[#09090B] shadow-inner">
                    ...
                    <div className="space-y-6">
                        {formData.outreach_logs && formData.outreach_logs.length > 0 ? (
                            formData.outreach_logs.map((log: any, idx: number) => (
                                ...
                                >
                                    ...
                                    {idx !== formData.outreach_logs.length - 1 && (
                                        <div className="absolute left-2 top-8 bottom-[-24px] w-px bg-slate-200 dark:bg-[#232734]"></div>
                                    )}
                                    {/* Timeline dot */}
                                    <div className="absolute left-0 top-1.5 w-4 h-4 rounded-full bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] flex items-center justify-center">
                                        <div className="w-1.5 h-1.5 rounded-full bg-slate-500"></div>
                                    </div>
                                    
                                    <div className="bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] rounded-[20px] p-5 shadow-sm">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-2">
                                                {getLogIcon(log.method || 'Note')}
                                                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{log.method || 'Note'}</span>
                                            </div>
                            ...
                            <div className="text-center py-12 bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] border-dashed rounded-3xl">
                        ...
                        {(formData.email_draft || formData.facebook_draft) && (
                            <div className="pt-8 mt-8 border-t border-slate-200 dark:border-[#232734]">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                                    <Sparkles size={14} /> Saved Drafts
                                </h3>
                                
                                {formData.email_draft && (
                                    <div className="bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] rounded-[20px] p-5 mb-4 shadow-sm">
                                        <h4 className="text-xs font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2"><Mail size={12} className="text-indigo-400"/> Email Draft</h4>
                                        <div className="text-xs text-slate-500 dark:text-slate-400 mb-2 font-semibold">Subject: {formData.email_subject_draft || 'No subject'}</div>
                                        <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-medium">{formData.email_draft}</p>
                                    </div>
                                )}

                                {formData.facebook_draft && (
                                    <div className="bg-white dark:bg-[#11131A] border border-slate-200 dark:border-[#232734] rounded-[20px] p-5 shadow-sm">
                                        <h4 className="text-xs font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2"><Globe size={12} className="text-blue-400"/> Facebook Draft</h4>
                                        <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-medium">{formData.facebook_draft}</p>
                                    </div>
                                )}
                            </div>
                        )}
            ...
            {/* Footer Actions */}
            <div className="p-6 border-t border-slate-200 dark:border-[#232734] bg-slate-50 dark:bg-[#09090B] flex justify-end gap-3 shrink-0">
              <button 
                type="button" 
                onClick={onClose}
                className="px-6 py-2.5 rounded-xl text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-white dark:bg-[#11131A] hover:bg-slate-200 dark:bg-[#232734] border border-slate-200 dark:border-[#232734] transition-all shadow-sm"
              >
                Cancel
              </button>
              <button 
