    ...
    <div className="space-y-8">
      ...
      {filteredClients.length === 0 ? (
        <div className="text-center py-20 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2rem]">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Add a new client to get started.</p>
        </div>
      ) : (
                ...
                <div className="space-y-4">
                  {(client.country || client.timezone) && (
                    <div className="flex items-center gap-4 p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                      {client.country && (
                        <div className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 font-semibold flex-1">
                          <Globe size={16} className="text-indigo-500" /> {client.country}
                        </div>
                      )}
                      {client.timezone && (
                        <div className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 font-semibold flex-1 border-l border-slate-200 dark:border-white/10 pl-4">
                  ...
                  {client.email && (
                    <div className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400 font-medium px-1">
            ...
            >
              ...
              <form onSubmit={handleSaveClient} className="p-6 md:p-8 space-y-6 overflow-y-auto max-h-[70vh] bg-slate-50/50 dark:bg-black/20">
                ...
                <div className="group relative">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Client Name *</label>
                  <div className="relative">
                ...
                <div className="pt-6 flex justify-end gap-3 border-t border-slate-200/50 dark:border-white/10">
                  <button 
                    type="button" 
                    onClick={closeModal}
                    className="px-6 py-3 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-white/10 transition-colors"
                  >
                    Cancel
                  </button>
                  <button 
                    ...
                    className="px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:shadow-lg hover:shadow-indigo-500/30 text-white font-bold rounded-xl transition-all disabled:opacity-50"
                  >
