...
function SortableRoadmapCard({ project, onClick }: { project: RoadmapProject, onClick?: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} onClick={(e) => {
      ...
      className={`bg-[#11131A] border border-[#232734] p-4 rounded-[20px] cursor-grab active:cursor-grabbing hover:border-[#2563EB]/40 transition-colors shadow-sm relative overflow-hidden group`}>
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${getStatusConfig(project.status).bg}`} />
      
      <div className="flex justify-between items-start mb-2 pl-2">
        <h4 className="text-sm font-bold text-white leading-snug line-clamp-2">{project.title}</h4>
      </div>
      ...
      <div className="pl-2">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-[10px] font-bold text-white font-mono">{prog}%</span>
        </div>
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="mb-8">
          <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5 mb-6">
            <div>
              ...
              <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight font-jakarta mb-1.5">Roadmap</h1>
            ...
            <div className="flex items-center bg-[#11131A] border border-[#232734] rounded-xl p-1 w-full md:w-auto overflow-x-auto hide-scrollbar">
              {[{ id: 'timeline', icon: Target, label: 'Roadmap' }, { id: 'kanban', icon: Columns, label: 'Kanban' }, { id: 'gantt', icon: AlignLeft, label: 'List' }].map(v => (
                <button key={v.id} onClick={() => setViewMode(v.id as ViewMode)}
                  className={`relative flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-colors ${viewMode === v.id ? 'text-white' : 'text-[#94A3B8] hover:text-[#94A3B8]/80'}`}>
                  {viewMode === v.id && <motion.div layoutId="roadmapViews" className="absolute inset-0 bg-[#232734] rounded-lg border border-white/5" />}
        ...
        {filteredProjects.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-32 flex flex-col items-center justify-center bg-[#11131A] border border-dashed border-[#232734] rounded-[20px]">
            <div className="w-16 h-16 rounded-[20px] bg-[#09090B] border border-[#232734] flex items-center justify-center mb-4"><Compass size={24} className="text-[#232734]" /></div>
            ...
            {viewMode === 'timeline' && (
              <motion.div key="timeline" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8 pb-8">
                {groupedByCategory.map(([category, items], idx) => (
                  <div key={category} className="bg-[#11131A] border border-[#232734] rounded-[24px] overflow-hidden">
                    <div className="px-6 py-4 border-b border-[#232734] bg-[#0D0F16]">
                      <h3 className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-widest">{category} Epic</h3>
                    </div>
                    <div className="p-6">
                      <div className="relative border-l-2 border-[#334155]/60 ml-3 space-y-6">
                        {items.map(p => {
                          ...
                          const nodeColor = isCompleted ? 'bg-[#10B981] border-[#10B981] shadow-[0_0_12px_rgba(16,185,129,0.5)]' : isPulse ? 'bg-[#2563EB] border-[#2563EB] shadow-[0_0_12px_rgba(37,99,235,0.8)]' : 'bg-[#09090B] border-[#475569] group-hover:border-[#2563EB]';
                          ...
                          return (
                            <div key={p._id} className="relative pl-8 group cursor-pointer" onClick={() => setSelectedProject(p)}>
                              ...
                              <div className="bg-[#09090B] border border-[#232734] rounded-[20px] p-5 hover:border-[#2563EB]/60 hover:shadow-[0_0_24px_rgba(37,99,235,0.15)] transition-all relative overflow-hidden">
                                ...
                                <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
                                  <div>
                                    <h4 className="text-base font-bold text-white group-hover:text-[#2563EB] transition-colors">{p.title}</h4>
                                    ...
                                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                                      ...
                                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-[#232734] border border-[#334155] text-[9px] font-bold text-white">
                                  ...
                                  <div className="w-full md:w-48 shrink-0 flex flex-col md:items-end">
                                    ...
                                    <div className="h-2 w-full bg-[#11131A] rounded-full overflow-hidden border border-[#232734] group-hover:border-[#2563EB]/40 transition-colors">
                  ...
                  <table className="w-full border-collapse text-sm">
                    <thead className="sticky top-0 z-10 bg-[#0D0F16]">
                      <tr className="border-b border-[#232734]">
                        <th className="pl-6 pr-4 py-4 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest w-[40%]">Milestone / Epic</th>
                        <th className="pr-4 py-4 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest">Status</th>
                        <th className="pr-6 py-4 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest w-[40%]">Progress</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          <motion.tr key={p._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}
                            ...
                            <td className="pl-6 pr-4 py-4">
                              <p className="font-bold text-white group-hover:text-[#2563EB] transition-colors">{p.title}</p>
                            ...
                            <td className="pr-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-[10px] font-bold text-white w-8 font-mono">{prog}%</span>
                              </div>
                            </td>
                          </motion.tr>
                        )
                ...
                <div className="flex-shrink-0 p-6 border-b border-[#232734] bg-[#11131A]">
                  ...
                  {isAddOpen ? (
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-[14px] bg-[#09090B] border border-[#232734] flex items-center justify-center text-[#2563EB] flex-shrink-0"><Target size={20} /></div>
                      <h2 className="text-lg font-bold text-white tracking-tight mt-1">Create a new milestone on the roadmap.</h2>
                    </div>
                  ) : selectedProject && (
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-12 h-12 rounded-[14px] bg-[#09090B] border border-[#232734] flex items-center justify-center text-[#2563EB] flex-shrink-0"><Target size={20} /></div>
                      <div className="flex-1 min-w-0">
                        <h2 className="text-lg font-bold text-white tracking-tight line-clamp-2">{selectedProject.title}</h2>
                  ...
                  <form onSubmit={handleCreate} className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-5">
                    <div>
                      ...
                      <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})}
                        className="w-full bg-[#11131A] border border-[#232734] text-white rounded-xl px-4 py-3 text-sm focus:border-[#2563EB]/60 focus:outline-none" placeholder="Milestone name..." />
                    </div>
                    <div>
                      ...
                      <input required type="text" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}
                        className="w-full bg-[#11131A] border border-[#232734] text-white rounded-xl px-4 py-3 text-sm focus:border-[#2563EB]/60 focus:outline-none" placeholder="e.g. Q3 Launch" />
                    </div>
                    <div>
                        ...
                        <div className="space-y-6">
                          ...
                          <form onSubmit={handleAddTask} className="flex gap-2">
                            <input type="text" placeholder="Add a new task..." value={newTaskText} onChange={e => setNewTaskText(e.target.value)}
                              className="flex-1 bg-[#11131A] border border-[#232734] text-white rounded-xl px-4 py-2.5 text-sm focus:border-[#2563EB]/60 focus:outline-none" />
                            <button type="submit" disabled={!newTaskText.trim() || isSubmitting}
                              className="px-4 bg-[#232734] hover:bg-[#2563EB] text-white rounded-xl transition-all disabled:opacity-50">
                              <Plus size={16} />
                            </button>
                          </form>
                          ...
                          <div className="space-y-2">
                            {selectedProject.logs?.map(log => (
                              <div key={log._id} className="group flex items-center justify-between p-3 bg-[#11131A] border border-[#232734] rounded-xl hover:border-[#2563EB]/30 transition-colors">
                                <label className="flex items-center gap-3 cursor-pointer flex-1 min-w-0">
                                  <input type="checkbox" checked={log.completed} onChange={(e) => toggleTask(log._id, e.target.checked)}
                                    className="w-4 h-4 rounded border-[#232734] text-[#2563EB] bg-[#09090B] focus:ring-[#2563EB] cursor-pointer" />
                                  <span className={`text-sm flex-1 truncate transition-all ${log.completed ? 'text-[#94A3B8] line-through' : 'text-white'}`}>{log.text}</span>
                                </label>
                            ...
                            {(!selectedProject.logs || selectedProject.logs.length === 0) && (
                              <div className="py-8 text-center border-2 border-dashed border-[#232734] rounded-xl">
