...
function SortableRoadmapCard({ project, onClick }: { project: RoadmapProject, onClick?: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} onClick={(e) => {
      ...
      className={`bg-white dark:bg-[#111111] border border-[#E2E8F0] dark:border-[#1a1a1a] p-4 rounded-[20px] cursor-grab active:cursor-grabbing hover:border-[#2563EB]/40 transition-colors shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none relative overflow-hidden group`}>
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="mb-8">
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
            {[
              { label: 'Total Milestones', value: projects.length, color: '#F59E0B' },
          ...
          <motion.div variants={itemVariants} className="flex flex-col md:flex-row items-center gap-3">
            <div className="flex items-center bg-slate-50 dark:bg-[#111111] border border-[#E2E8F0] dark:border-[#1a1a1a] rounded-xl p-1 w-full md:w-auto overflow-x-auto hide-scrollbar shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none">
              {[{ id: 'timeline', icon: Target, label: 'Roadmap' }, { id: 'kanban', icon: Columns, label: 'Kanban' }, { id: 'gantt', icon: AlignLeft, label: 'List' }].map(v => (
                <button key={v.id} onClick={() => setViewMode(v.id as ViewMode)}
                  ...
                  {viewMode === v.id && <motion.div layoutId="roadmapViews" className="absolute inset-0 bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm dark:shadow-none border border-[#E2E8F0] dark:border-white/5" />}
            ...
            <div className="flex-1 w-full flex items-center gap-3">
              <div className="relative flex-1 group">
                ...
                <input type="text" placeholder="Search roadmap..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  className="w-full bg-white dark:bg-[#111111] border border-[#E2E8F0] dark:border-[#1a1a1a] text-slate-900 dark:text-white placeholder-[#94A3B8]/60 text-sm font-medium rounded-xl pl-11 pr-10 py-2.5 focus:outline-none focus:border-[#2563EB]/60 focus:ring-2 focus:ring-[#2563EB]/10 transition-all shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none" />
              </div>
              <div className="relative w-32 shrink-0">
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                  className="w-full bg-white dark:bg-[#111111] border border-[#E2E8F0] dark:border-[#1a1a1a] text-slate-900 dark:text-white text-sm font-medium rounded-xl pl-4 pr-8 py-2.5 appearance-none focus:outline-none focus:border-[#2563EB]/60 cursor-pointer shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none">
        ...
        {filteredProjects.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-32 flex flex-col items-center justify-center bg-white dark:bg-[#111111] border border-dashed border-[#E2E8F0] dark:border-[#1a1a1a] rounded-[20px]">
            <div className="w-16 h-16 rounded-[20px] bg-slate-50 dark:bg-[#09090B] border border-[#E2E8F0] dark:border-[#1a1a1a] flex items-center justify-center mb-4"><Compass size={24} className="text-[#94A3B8]" /></div>
          ...
          <AnimatePresence mode="wait">
            ...
            {viewMode === 'timeline' && (
              <motion.div key="timeline" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8 pb-8">
                {groupedByCategory.map(([category, items], idx) => {
                  const isExpanded = !!expandedEpics[category];
                  
                  // Calculate epic progress
                  const totalLogs = items.reduce((acc, p) => acc + (p.logs?.length || 0), 0);
                  const compLogs = items.reduce((acc, p) => acc + (p.logs?.filter(l => l.completed).length || 0), 0);
                  const epicProg = totalLogs === 0 ? 0 : Math.round((compLogs / totalLogs) * 100);

                  return (
                    <div key={category} className="bg-white dark:bg-[#111111] border border-[#E2E8F0] dark:border-[#1a1a1a] rounded-[24px] overflow-hidden shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none transition-all">
                      <div 
                        className={`relative px-6 py-4 flex items-center justify-between cursor-pointer transition-colors group overflow-hidden ${isExpanded ? 'bg-slate-50 dark:bg-[#09090B] border-b border-[#E2E8F0] dark:border-[#1a1a1a]' : 'hover:bg-slate-50 dark:hover:bg-[#141414]'}`}
                        onClick={() => setExpandedEpics(prev => ({ ...prev, [category]: !prev[category] }))}
                      >
                        {/* Hover Gradient Background */}
                        <div className="absolute inset-0 bg-gradient-to-r from-[#2563EB]/0 via-[#2563EB]/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                        
                        <div className="relative flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${isExpanded ? 'bg-[#2563EB] text-white shadow-[0_0_12px_rgba(37,99,235,0.4)]' : 'bg-slate-100 dark:bg-[#1a1a1a] text-[#94A3B8] group-hover:text-[#2563EB]'}`}>
                            <Layers size={14} />
                          </div>
                          <div>
                            <h3 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-widest">{category} Epic <span className="text-[#94A3B8] ml-2 normal-case font-medium tracking-normal text-[11px]">{items.length} projects</span></h3>
                          </div>
                        </div>
                        ...
                        <div className="relative flex items-center gap-6">
                          ...
                          <div className="hidden sm:flex items-center gap-3 w-32">
                            <div className="flex-1 h-1.5 bg-slate-200 dark:bg-[#232734] rounded-full overflow-hidden">
                              <div className="h-full bg-[#10B981] rounded-full transition-all duration-500" style={{ width: `${epicProg}%` }} />
                            </div>
                            <span className="text-[10px] font-bold text-[#94A3B8] font-mono">{epicProg}%</span>
                          </div>

                          <div className={`p-1.5 rounded-lg transition-colors ${isExpanded ? 'bg-slate-200 dark:bg-[#232734] text-slate-900 dark:text-white' : 'text-[#94A3B8] group-hover:bg-slate-200 dark:group-hover:bg-[#232734] group-hover:text-slate-900 dark:group-hover:text-white'}`}>
                      ...
                      {isExpanded && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="p-6">
                          <div className="relative border-l-2 border-[#E2E8F0] dark:border-[#1a1a1a] ml-3 space-y-6">
                            {items.map(p => {
                              ...
                              const nodeColor = isCompleted ? 'bg-[#10B981] border-[#10B981] shadow-[0_0_12px_rgba(16,185,129,0.5)]' : isPulse ? 'bg-[#2563EB] border-[#2563EB] shadow-[0_0_12px_rgba(37,99,235,0.8)]' : 'bg-slate-50 dark:bg-[#09090B] border-[#94A3B8] group-hover:border-[#2563EB]';
                              ...
                              return (
                                <div key={p._id} className="relative pl-8 group cursor-pointer" onClick={() => setSelectedProject(p)}>
                                  ...
                                  <div className="bg-white dark:bg-[#111111] border border-[#E2E8F0] dark:border-[#1a1a1a] rounded-[20px] p-5 hover:border-[#2563EB]/60 hover:shadow-md shadow-sm dark:shadow-none transition-all relative overflow-hidden">
                                    ...
                                    <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
                                      <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                                          ...
                                          <span className="w-1 h-1 rounded-full bg-slate-300 dark:bg-[#1a1a1a]" />
                                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-slate-50 text-slate-600 border border-[#E2E8F0] dark:bg-[#09090B] dark:text-slate-400 dark:border-[#1a1a1a] text-[9px] font-bold">
                                      ...
                                      <div className="w-full md:w-48 shrink-0 flex flex-col md:items-end">
                                        ...
                                        <div className="h-2 w-full bg-slate-50 dark:bg-[#09090B] rounded-full overflow-hidden border border-[#E2E8F0] dark:border-[#1a1a1a] group-hover:border-[#2563EB]/40 transition-colors">
            ...
            {viewMode === 'gantt' && (
              <motion.div key="gantt" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className="overflow-hidden rounded-[20px] border border-[#E2E8F0] dark:border-[#1a1a1a] bg-white dark:bg-[#111111] shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none">
                      ...
                      {filteredProjects.map((p, i) => {
                        ...
                        return (
                          <motion.tr key={p._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}
                            ...
                            className="border-b border-[#E2E8F0] dark:border-[#1a1a1a]/60 cursor-pointer hover:bg-slate-50 dark:hover:bg-[#141414] transition-colors group">
                            ...
                            <td className="pr-6 py-4">
                              <div className="flex items-center gap-3">
                                <div className="flex-1 h-1.5 bg-slate-50 dark:bg-[#09090B] rounded-full overflow-hidden border border-[#E2E8F0] dark:border-[#1a1a1a]">
                                  <div className="h-full bg-[#2563EB] rounded-full" style={{ width: `${prog}%` }} />
                                </div>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 lg:p-10 pointer-events-none">
                ...
                <div className="flex-shrink-0 p-6 border-b border-[#E2E8F0] dark:border-[#1a1a1a] bg-white dark:bg-[#111111]">
                  ...
                  {isAddOpen ? (
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-[14px] bg-slate-50 dark:bg-[#09090B] border border-[#E2E8F0] dark:border-[#1a1a1a] flex items-center justify-center text-[#2563EB] flex-shrink-0"><Target size={20} /></div>
                  ...
                  <form onSubmit={handleCreate} className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-6 space-y-5">
                    <div>
                      ...
                      <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})}
                        className="w-full bg-slate-50 dark:bg-[#09090B] border border-[#E2E8F0] dark:border-[#1a1a1a] text-slate-900 dark:text-white rounded-xl px-4 py-3 text-sm focus:border-[#2563EB]/60 focus:outline-none shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none" placeholder="Milestone name..." />
                    </div>
                    <div>
                      ...
                      <input required type="text" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}
                        className="w-full bg-slate-50 dark:bg-[#09090B] border border-[#E2E8F0] dark:border-[#1a1a1a] text-slate-900 dark:text-white rounded-xl px-4 py-3 text-sm focus:border-[#2563EB]/60 focus:outline-none shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none" placeholder="e.g. Q3 Launch" />
                    </div>
                    <div>
                ...
                {selectedProject && !isAddOpen && (
                  <div className="flex-1 flex flex-col overflow-hidden">
                    {/* Tabs */}
                    <div className="flex-shrink-0 flex items-center border-b border-[#E2E8F0] dark:border-[#1a1a1a] px-6 bg-slate-50 dark:bg-[#09090B]">
                    ...
                    <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-6 bg-white dark:bg-[#111111]">
                      {panelTab === 'tasks' && (
                        <div className="space-y-6">
                          {/* Add Task Input */}
                          <form onSubmit={handleAddTask} className="flex gap-2">
                            <input type="text" placeholder="Add a new task..." value={newTaskText} onChange={e => setNewTaskText(e.target.value)}
                              className="flex-1 bg-slate-50 dark:bg-[#09090B] border border-[#E2E8F0] dark:border-[#1a1a1a] text-slate-900 dark:text-white rounded-xl px-4 py-2.5 text-sm focus:border-[#2563EB]/60 focus:outline-none shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none" />
                            <button type="submit" disabled={!newTaskText.trim() || isSubmitting}
                              className="px-4 bg-slate-100 dark:bg-[#1a1a1a] hover:bg-[#2563EB] text-[#94A3B8] hover:text-white rounded-xl transition-all disabled:opacity-50">
                              <Plus size={16} />
                            </button>
                          </form>
                          ...
                          <div className="space-y-2">
                            {selectedProject.logs?.map(log => (
                              <div key={log._id} className="group flex items-center justify-between p-3 bg-slate-50 dark:bg-[#09090B] border border-[#E2E8F0] dark:border-[#1a1a1a] rounded-xl hover:border-[#2563EB]/30 transition-colors shadow-[0_2px_10px_-2px_rgba(0,0,0,0.02)] dark:shadow-none">
                                <label className="flex items-center gap-3 cursor-pointer flex-1 min-w-0">
                                  <input type="checkbox" checked={log.completed} onChange={(e) => toggleTask(log._id, e.target.checked)}
                                    className="w-4 h-4 rounded border-[#E2E8F0] dark:border-[#1a1a1a] text-[#2563EB] bg-white dark:bg-[#111111] focus:ring-[#2563EB] cursor-pointer" />
                            ...
                            {(!selectedProject.logs || selectedProject.logs.length === 0) && (
                              <div className="py-8 text-center border-2 border-dashed border-[#E2E8F0] dark:border-[#1a1a1a] rounded-xl">
