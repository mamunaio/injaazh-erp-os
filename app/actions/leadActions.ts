...
export async function getLastSenderForLead(leadId: string) {
  try {
    ...
    const log = await EmailCampaignLog.findOne({ leadId, status: 'Sent' }).sort({ sentAt: -1 }).lean();
...
export async function sendOutreachEmail(leadId: string, subject: string, body: string, senderAccountId: string = 'auto') {
  try {
    ...
    if (senderAccountId === 'auto') {
      ...
      const previousLog = await EmailCampaignLog.findOne({ leadId: lead._id, status: 'Sent' }).sort({ sentAt: -1 }).lean();
